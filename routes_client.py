...
async def client_jobs(user=Depends(require_role("client")), conn=Depends(getDB)):
    ...
    async with conn.cursor() as cur:
        await cur.execute(
            """
            SELECT j.id, j.title, j.status, j.created_at, j.contractor_id, -- 記得多撈 contractor_id
                   COALESCE((SELECT COUNT(*) FROM bids b WHERE b.job_id = j.id), 0) AS bid_count,
                   u_con.username AS contractor_name,
                   -- 新增：計算得標承包人(contractor)的平均評價
                   (
                       SELECT AVG((rating_1 + rating_2 + rating_3)/3.0)
                       FROM reviews r
                       WHERE r.to_user_id = j.contractor_id AND r.target_role = 'contractor'
    ...
    try:
        async with conn.transaction():
            async with conn.cursor() as cur:
                ...
                # 若有設定截止日，必須到了之後才能選標；若無設定(None)，則隨時可選標
